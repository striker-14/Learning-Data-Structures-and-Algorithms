/*
Q. Union (# common and distinct elements in the two array) of Two Sorted Arrays.

# Elements in union should be in ascending order.
*/
...
vector<int> unionSortedArrays(int arr1[], int arr2[], int n, int m) {
    ...
    while (i < n && j < m) {
        if (arr1[i] <= arr2[j]) {
            ...
            i++; // # Notice the position of i++, if duplicate element, it moves to next element (if the if condition is satisfied)
        } else {
